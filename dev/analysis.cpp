...
analysis::~analysis()
{
    if (evt_cut != nullptr) {delete evt_cut;}
    if (first_track_cut != nullptr) {delete first_track_cut;}
    if (second_track_cut != nullptr) {delete second_track_cut;}
    if (pr_cut != nullptr) {delete pr_cut;}

    for (auto corr : *correlations) {delete corr;}
    correlations->clear();
    delete correlations;

    for (auto evt : *event_mixing_buffer) {delete evt;}
...
void analysis::process(const event *evt)
{
    ...
    if (this->evt_cut && !this->evt_cut->pass(evt)) {return;}
...
void analysis::fill_real_pair_correlation(particle_collection* first, particle_collection* second) {
    ...
    for (auto& iptcl = start_outer; iptcl != end_outer; iptcl++) {
        for (auto& jptcl = start_inner; jptcl != end_inner; jptcl++) {
            ...
            if (pr_cut == nullptr || pr_cut->pass(pr)) {

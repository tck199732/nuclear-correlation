...
event *custom_reader::read() {
	...
	for (int j = 0; j < branches.multi; j++) {
		...
		auto trk = new custom_track(branches.N[j], branches.Z[j], branches.vx[j], branches.vy[j],
									branches.vz[j], mass);
		...
		// if possible, always use this method, though it needs `std::any_cast` to retrieve the
		// value

...
public:
    ...
    void set_accepted_trigger_condition(const std::string &v) { this->accepted_trigger_condition = v; }
    void set_accepted_trigger_value(const double &vmin, const double &vmax) { this->accepted_trigger_value = {vmin, vmax}; }

    std::array<unsigned int, 2> get_accepted_multiplicity() const { return accepted_multiplicity; }
    std::array<double, 2> get_accepted_impact_parameter() const { return accepted_impact_parameter; }

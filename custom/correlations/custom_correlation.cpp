...
void custom_correlation::set_numerator(const std::string &name, const std::string &title,
									   const int &bins, const double &vmin, const double &vmax) {
	if (this->numerator) {
...
void custom_correlation::set_denominator(const std::string &name, const std::string &title,
										 const int &bins, const double &vmin, const double &vmax) {
	if (this->denominator) {
...
void custom_correlation::add_real_pair(const pair *pr) {
	...
	double px2 = second->get_px(), py2 = second->get_py(), pz2 = second->get_pz(),
		   e2 = second->get_e();
